...
class WalletKeys:
    """Manages wallet keypair storage"""
    ...
    def generate_and_save(self) -> Tuple[nacl.signing.SigningKey, nacl.signing.VerifyKey]:
        """Generate new wallet keys and save them"""
        signing_key = nacl.signing.SigningKey.generate()
        verify_key = signing_key.verify_key
        
        # Save private key
        private_path = os.path.join(self.storage_dir, "wallet_private.key")
        with open(private_path, 'w') as f:
            f.write(signing_key.encode(encoder=nacl.encoding.Base64Encoder).decode('utf-8'))
        
        # Save public key
        public_path = os.path.join(self.storage_dir, "wallet_public.key")
    ...
    def load_signing_key(self) -> Optional[nacl.signing.SigningKey]:
        ...
        path = os.path.join(self.storage_dir, "wallet_private.key")
    ...
    def load_verify_key(self) -> Optional[nacl.signing.VerifyKey]:
        """Load wallet verification key"""
        path = os.path.join(self.storage_dir, "wallet_public.key")
        if not os.path.exists(path):
            return None
        with open(path, 'r') as f:
            key_data = f.read().strip()
